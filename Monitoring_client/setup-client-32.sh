...
# ============================================================================ #
	cd $ROOT;
	if [ ! -f lm-sensors-3-5-0.tar.gz ]; then
		wget --no-check-certificate -4 https://fossies.org/linux/misc/lm-sensors-3-5-0.tar.gz;
	fi;
	if [ ! -f lm-sensors-3-5-0.tar.gz ]; then
		echo "[ERROR] File not found: " lm-sensors-3-5-0.tar.gz;
		exit 1;
	fi;
