...
char* mf_plugin_dummy_hook() {
	if (is_initialized) {
		/*
		* sampling 
		*/
		mf_dummy_sample(monitoring_data);
		...
		*/
		...
		mf_dummy_to_json(monitoring_data, json);
		return json;
	} else
		return NULL;
}
...
extern int init_mf_plugin_dummy(PluginManager *pm) {
	/*
	* get the turned on metrics from the configuration file
	*/
	conf_data =  malloc(sizeof(mfp_data));
	mfp_get_data_filtered_by_value("mf_plugin_dummy", conf_data, "on");

	/*
	* initialize the monitoring data
	*/
	monitoring_data = malloc(sizeof(Plugin_metrics));
	int ret = mf_dummy_init(monitoring_data, conf_data->keys, conf_data->size);
	if(ret == 0) {
		char plugin_name[] = "dummy";
	...
	*/
	PluginManager_register_hook(pm, "mf_plugin_dummy", mf_plugin_dummy_hook);
	is_initialized = 1;
	return ret;
}
